...
/*auto Vertex::get_bkmers( Vertex* v )
    CO2_BEG(co2::recursive_generator< char* >, ( v ),
        std::set< Bkmer >* uc_bkmers;
        std::set< Bkmer >::iterator uc_bkmer;
        std::vector< std::unique_ptr< CContainer > >* ccs;
        std::vector< std::unique_ptr< CContainer > >::iterator cc;
        Vertex* child_vertex;
...
{
    uc_bkmers = v->m_uc->get_bkmers();
    uc_bkmer = uc_bkmers->begin();
    std::cout << "uc bkmers size: " << uc_bkmers->size() << std::endl << std::flush;
    //tmp = ( char* ) calloc( 4, sizeof( char ) );
    for( ; uc_bkmer != uc_bkmers->end(); uc_bkmer++ )
    {
        //std::cout << "\t\tinside uc: " << ( *uc_bkmer ).get_seq() << std::endl << std::flush;
        //std::cout << "\t\t\tafter print" << std::endl << std::flush;
        //CO2_YIELD( *uc_bkmer );
        //CO2_YIELD( tmp );
        CO2_YIELD( uc_bkmer->get_seq() );
        //std::cout << "\t\t\tdone returning" << std::endl << std::flush;
    }

    //std::cout << "\tDone with UC!" << std::endl << std::flush;

    ccs = v->m_ccs;
    cc = ccs->begin();
    for( ; cc != ccs->end(); cc++ )
    {
        //std::cout << "\tstarting CC..." << std::endl << std::flush;
        sfc = ( *cc ).get()->get_suf_clust_data();
        clust = sfc->begin();
        for( ; clust != sfc->end(); clust++ )
        {
            //std::cout << "\t\tcluster..." << std::endl << std::flush;
            child_vertex = ( *clust )->get_child_vertex();
            CO2_YIELD( get_bkmers( child_vertex ) );
            //std::cout << "\t\tDone with cluster" << std::endl << std::flush;
            //CO2_YIELD( get_bkmers( v ) );
        }
        //std::cout << "\tDone with CC" << std::endl << std::flush;
    }
